...
import com.zkp.wanandroid.utils.Util;
...
public class SuitLines extends View {
    ...
    public SuitLines(Context context, AttributeSet attrs) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.SuitLines);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                lastX = event.getX();
                velocityTracker.addMovement(event);
                if (needEdgeEffect && Objects.requireNonNull(datas.get(0)).size() > maxOfVisible) {
            ...
            case MotionEvent.ACTION_UP:
                if (needShowHint && event.getAction() == MotionEvent.ACTION_UP) {
                    boolean canCallTap = Math.abs(event.getX() - firstX) < 2 && Math.abs(event.getY() - firstY) < 2;
                    if (canCallTap) {
    ...
    private void calcUnitXY() {
        ...
        int realNum = Math.min(Objects.requireNonNull(datas.get(0)).size(), maxOfVisible);
        realBetween = linesArea.width() / (realNum - 1);
        // 防止line的stroke部分在lineArea外被clip
        float padding = paints.get(0).getStrokeWidth() / 2;
        for (int i = 0; i < Objects.requireNonNull(datas.get(0)).size(); i++) {
            for (int j = 0; j < datas.size(); j++) {
                float curValue = Objects.requireNonNull(datas.get(j)).get(i).getValue();
                ...
                Objects.requireNonNull(datas.get(j)).get(i).setXY(new PointF(linesArea.left + realBetween * i,
                        linesArea.top + linesArea.height() * scale + (scale == 0 ? padding : (scale == 1 ? -padding : 0))));
                if (i == Objects.requireNonNull(datas.get(0)).size() - 1) {
                    maxOffset = Math.abs(Objects.requireNonNull(datas.get(j)).get(i).getXY().x) - linesArea.width() - linesArea.left;
                }
            }
        }
    ...
    private boolean noNeedCalcEdge(float offset) {
        return suitEdge != null
                && Objects.requireNonNull(datas.get(0)).get(suitEdge[0]).getXY().x <= linesArea.left - offset
                && Objects.requireNonNull(datas.get(0)).get(suitEdge[1]).getXY().x >= linesArea.right - offset;
    }
    ...
    private void drawExsitDirectly(Canvas canvas) {
        for (int j = 0; j < datas.size(); j++) {
            if (!isLineFill() || !needCoverLine) {
                ...
                tmpPath.lineTo(Objects.requireNonNull(datas.get(j)).get(suitEdge[1]).getXY().x, linesArea.bottom);
                tmpPath.lineTo(Objects.requireNonNull(datas.get(j)).get(suitEdge[0]).getXY().x, linesArea.bottom);
                tmpPath.close();
    ...
    private void drawClickHint(Canvas canvas) {
        Unit cur = Objects.requireNonNull(datas.get(clickIndexs[1])).get(clickIndexs[0]);
        canvas.drawLine(Objects.requireNonNull(datas.get(clickIndexs[1])).get(suitEdge[0]).getXY().x, cur.getXY().y,
                Objects.requireNonNull(datas.get(clickIndexs[1])).get(suitEdge[1]).getXY().x, cur.getXY().y, hintPaint);
        ...
        if (!TextUtils.isEmpty(cur.getExtX())) {
            canvas.drawText("日期: " + cur.getExtX(), bak.centerX(), bak.centerY() - 12, hintPaint);
        }
        canvas.drawText("温度: " + cur.getValue() + "℃", bak.centerX(),
    ...
    private int[] findSuitEdgeInVisual2() {
        ...
        if (offset == 0) {// 不可滑动或当前位于最左边
            startIndex = 0;
            endIndex = Math.min(Objects.requireNonNull(datas.get(0)).size() - 1, maxOfVisible - 1);
        } else if (Math.abs(offset) == maxOffset) {// 可滑动且当前位于最右边
            endIndex = Objects.requireNonNull(datas.get(0)).size() - 1;
    ...
    private void drawLines(Canvas canvas, int startIndex, int endIndex) {
        ...
        for (int i = startIndex; i <= endIndex; i++) {
            for (int j = 0; j < datas.size(); j++) {
                Unit current = Objects.requireNonNull(datas.get(j)).get(i);
                ...
                if (lineType == SEGMENT) {
                    ...
                    // 到这里肯定不是起始点，所以可以减1
                    Unit previous = Objects.requireNonNull(datas.get(j)).get(i - 1);
    ...
    private void drawX(Canvas canvas, int startIndex, int endIndex) {
        canvas.drawLine(Objects.requireNonNull(datas.get(0)).get(startIndex).getXY().x, xArea.top,
                Objects.requireNonNull(datas.get(0)).get(endIndex).getXY().x, xArea.top, xyPaint);
        String extX, currentDate;
        for (int i = startIndex; i <= endIndex; i++) {
            extX = Objects.requireNonNull(datas.get(0)).get(i).getExtX();
            if (i > 0) {
                currentDate = Objects.requireNonNull(datas.get(0)).get(i - 1).getExtX().substring(0, 5);
                extX = getUnitTitle(currentDate, extX);
            }
            ...
            } else if (i == endIndex && endIndex == Objects.requireNonNull(datas.get(0)).size() - 1) {
                xyPaint.setTextAlign(Paint.Align.RIGHT);
            } else {
                xyPaint.setTextAlign(Paint.Align.CENTER);
            }
            canvas.drawText(extX, Objects.requireNonNull(datas.get(0)).get(i).getXY().x, Util.calcTextSuitBaseY(xArea, xyPaint), xyPaint);
            canvas.drawLine(Objects.requireNonNull(datas.get(0)).get(i).getXY().x, xArea.top,
                    Objects.requireNonNull(datas.get(0)).get(i).getXY().x, xArea.top + basePadding, xyPaint);
        }
    }
        ...
        if (yAreaBuffer == null) {
            ...
            for (int i = 0; i < countOfY; i++) {
                ...
                if (i == 0) {
                    ...
                    y = yAxis = yRect.bottom - yRect.height() * 1.0f / (countOfY - 1) * i + Util.getTextHeight(xyPaint) / 2;
                }
        ...
        if (yGridBuffer == null) {
            ...
            for (int i = 0; i < countOfY; i++) {
                ...
                if (i == 0) {
                    ...
                    yAxis = yRect.bottom - yRect.height() * 1.0f / (countOfY - 1) * i + Util.getTextHeight(xyPaint) / 2;
                }
    ...
    public String getUnitTitle(String currentDate, String title) {
        String result;

        if (title.contains(currentDate)) {
            result = title.replace(currentDate, "");
        } else {
            result = title;
        }
    ...
    private void onTap(float upX, float upY) {
        ...
        if (realIndex != -1) {
            ...
            for (int i = 0; i < datas.size(); i++) {
                float cur = Math.abs(Objects.requireNonNull(datas.get(i)).get(realIndex).getXY().y - upY);
                if (cur <= clickSlop) {
                    if (mostMatchY != -1) {
                        if (Math.abs(Objects.requireNonNull(datas.get(mostMatchY)).get(realIndex).getXY().y - upY) > cur) {
                            mostMatchY = i;
                        }
                    } else {
                        mostMatchY = i;
            ...
            if (mostMatchY != -1) {
                ...
                //设置提示信息展示时长：3秒
                clickHintAnimator.setDuration(3000);
                clickHintAnimator.setInterpolator(linearInterpolator);
                clickHintAnimator.addUpdateListener(animation -> {
    ...
    private int[] findSuitEdgeInVisual() {
        int startIndex = 0, endIndex = Objects.requireNonNull(datas.get(0)).size() - 1;
        if (offset == 0) {// 不可滑动或当前位于最左边
            endIndex = Math.min(Objects.requireNonNull(datas.get(0)).size() - 1, maxOfVisible - 1);
        } else if (Math.abs(offset) == maxOffset) {// 可滑动且当前位于最右边
            endIndex = Objects.requireNonNull(datas.get(0)).size() - 1;
            ...
            if (Objects.requireNonNull(datas.get(0)).size() > maxOfVisible) {
                // 找到指定区间的第一个被发现的点
                int suitKey = 0;
                int low = 0;
                int high = Objects.requireNonNull(datas.get(0)).size() - 1;
                ...
                while (suitKey >= 0) {
                    startIndex = suitKey;
                    if (Objects.requireNonNull(datas.get(0)).get(suitKey).getXY().x <= startX) {
                        break;
                    }
                    suitKey--;
                }
                suitKey = bakKey;
                // 再右边
                while (suitKey < Objects.requireNonNull(datas.get(0)).size()) {
                    endIndex = suitKey;
                    if (Objects.requireNonNull(datas.get(0)).get(suitKey).getXY().x >= endX) {
                        break;
                    }
                    suitKey++;
                }
    ...
    private float calcReferenceLengthOf(int j) {
        return linesArea.height() * 2 - Objects.requireNonNull(datas.get(j)).get(suitEdge[0]).getXY().y
                - Objects.requireNonNull(datas.get(j)).get(suitEdge[1]).getXY().y
                + Objects.requireNonNull(datas.get(j)).get(suitEdge[1]).getXY().x - Objects.requireNonNull(datas.get(j)).get(suitEdge[0]).getXY().x;
    }
    ...
    private void calcMaxUnit(Map<Integer, List<Unit>> datas) {
        ...
        minAndMaxOfY[0] = minUnit.getValue() - 1;
        minAndMaxOfY[1] = maxUnit.getValue() + 1;

        countOfY = (int) (minAndMaxOfY[1] - minAndMaxOfY[0]) + 1;
    }
    ...
    private long calcTotalCost() {
        if (datas.isEmpty() || Objects.requireNonNull(datas.get(0)).isEmpty()) {
            return 0;
        }
    ...
    private long calcVisibleLineCost() {
        if (intervalOfAnimCost > 0) {
            if (maxOfVisible < Objects.requireNonNull(datas.get(0)).size()) {
                return maxOfAnimCost;
            }
            long oneLineCost = intervalOfAnimCost * (Objects.requireNonNull(datas.get(0)).size() - 1);
    ...
    private void autoInvalidate() {
        ...
        handler.postDelayed(this::autoInvalidate, 16);
    }
    ...
    public void feedWithAnim(List<Unit> line) {
        ...
        @SuppressLint("UseSparseArrays") final Map<Integer, List<Unit>> entry = new HashMap<>();
        entry.put(0, line);
        handler.post(() -> feedInternal(entry, Collections.singletonList(buildNewPaint()), true));
    }
    ...
    public void feed(List<Unit> line) {
        ...
        @SuppressLint("UseSparseArrays") final Map<Integer, List<Unit>> entry = new HashMap<>();
        entry.put(0, line);
        handler.post(() -> feedInternal(entry, Collections.singletonList(buildNewPaint()), false));
    }
    ...
    public static class LineBuilder {
        ...
        public void build(final SuitLines suitLines, final boolean needAnim) {
            ...
            for (int i = 0; i < colors.size(); i++) {
                ...
                paint.setColor(Objects.requireNonNull(colors.get(0))[0]);
                paint.setShader(suitLines.buildPaintColor(colors.get(i)));
                tmpPaints.add(i, paint);
            }
            suitLines.postAction(() -> suitLines.feedInternal(datas, tmpPaints, needAnim));
        }
    }
}
